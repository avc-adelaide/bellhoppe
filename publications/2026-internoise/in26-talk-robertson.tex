...
\usepackage{booktabs,refstyle,siunitx,multicol}
...
This paper introduces \texttt{aubellhop} \parencite{robertson2026-bellhop}, a modern Python interface for Bellhop,
an open source Fortran program for calculating underwater acoustic phenomena \cite{porter2025-foa} using ray tracing and beam tracing algorithms (\Figref{erays}).
Bellhop is written and maintained by Michael B. Porter, with early development in the 1980s (first release 1983 with Homer Bucker, Naval Ocean Systems Center), and continuous and active development through the 2000s and 2010s as part of the Acoustic Toolbox \cite{porter2024-at}.
...
The Bellhop codebase in the Acoustics Toolbox includes both a wrapper and a translation of the code into Matlab, and wrappers for Bellhop have been provided in packages for Python \cite{chitre2025-arlpy,rodriguez2026-pyat} and Julia \cite{chitre2025-jl}.
...
Others have provided detailed graphical user interfaces for Bellhop \cite{duncan2025-actup}, which are useful from a pedagogical and ease-of-use perspective.
...
With modern implementations and wrappers, we might question whether the original source code is still of relevance.
The compiled Fortran code runs an order of magnitude faster than the Matlab code, and the C++/CUDA reimplementations \parencite{jaffe2025-bellhopcuda} can be orders of magnitude faster again.
The C++/CUDA reimplementation of Fortran maintains strict compatibility with the Fortran interfaces in terms of input and output files.
However, the executables for Bellhop use an unstructured plain text approach to setting up simulation parameters.
...
Providing a more useful interface for controlling Bellhop's many input parameters is a significant part of the \texttt{aubellhop} codebase.

Other work in Bellhop Python interfaces include PYAT by \textcite{rodriguez2026-pyat} and \texttt{arlpy} \textcite{chitre2025-arlpy}.
The approach of PYAT is to echo the original Fortran interfaces and to streamline the execution of its example files, but it does not provide a `user friendly' interface and requires the Acoustic Toolbox to be installed separately.
The \texttt{arlpy} interface provides a modern interface, but it is no longer under active development and only supports a subset of Bellhop's features.
...
\end{verbatim}
\normalsize
The eigenrays of this environment can be calculated using \verb|erays = bh.compute_eigenrays(env)| (see \Figref{erays}).
If you have legacy Bellhop configuration files in use (say, a \texttt{seamount} environment file), you can load it directly using \verb|bh.Environment.from_file('seamount.env')|.
The \texttt{Environment} class provides a number of helper functions to help manipulate the data, including serialisation to/from disk or data structures.
...
\texttt{aubellhop} has the capability to execute different binaries according to the needs of the model.
This allows direct support for Bellhop3D, and also facilitates comparison between multiple engines such as \texttt{bellhopcuda}, which are gaining support due to the multithreaded and GPU-accelerated operation, with example applications in studying 3D acoustic refraction around seamounts \cite{jerome2025-jasa}.
...
\begin{tabular}[t]{l}
    ...
    \MIDTITLE{ Francois-Garrison volume attenuation\textsuperscript\textdagger}    \\
    _fg_salinity                                        \\
    _fg_temperature                                     \\
    _fg_pH                                              \\
    _fg_depth                                           \\
...
\raggedright
 \footnotesize
\emph{\textsuperscript\textdagger denoted as internal as there is a separate `setter' function}
\end{minipage}
\end{table}
...
In this example, we use sound speed data derived from the NOAA World Ocean Atlas by Brian Dushaw \parencite{dushaw2015-woa}. A world-wide horizontal slices of this data at constant depth is shown in \Figref{lev_ssp}, with a vertical slice for constant longitude shown in \Figref{lev_ssp_long}. The code to perform the data manipulation into Python DataFrame format is not interesting here (see the repository page for a worked example), but once this is set up an eigenray analysis with 2D sound speed data can be undertaken with:
\small
\begin{verbatim}
...
\end{verbatim}
\normalsize
As the reflections off the ice are not governed by standard boundary conditions, addition settings are provided to capture these behaviours correctly.
...
Nonetheless, it is a `legacy' codebase in the sense that it is reliable and stable, and does not (need to) make use of modern software development practices.
The following key processes were added to modernise the Bellhop codebase.
...
FORD analyses the code and creates connected graphs (\Figref{graphvis}) to link files to files, modules to modules, and so on. Most importantly, this showed which source files were not being used in the Bellhop codebase, as the Fortran sources are shared alongside additional underwater acoustics programs -- Kraken, etc.\ -- with a shared library in the Acoustics Toolbox.

The Python interface to bellhop is documented using Sphinx (following \texttt{arlpy}). The Sphinx output is combined with the main FORD output to create the documentation pages.
...
The Bellhop repository is set up with a commandline interface provided by Make to help streamline shifting from one build process to another in the future.
...
Building engineering software using automatic documentation, checking, and testing tools helps mitigate the risk of unknown errors in the code, promotes reproducibility, and largely comes `for free' using modern programming tools and libraries.
