...
Initially this feature was added to support Bellhop3D, but it also facilitates comparison between multiple engines.
Setting up a 3D environment is identical to a 2D environment, with some additional parameters possible to define 3D speed of sound profiles, cross-range bathymetries, and so on.
...
env3d = bh.Environment(
                       name="Example Bellhop3D",
                       dimension="3D",
                       beam_angle_min=-45,
                       beam_angle_max=45,
                       beam_num=10_000,
...
\begin{figure}
\centerline{\fbox{\includegraphics[width=0.8\linewidth]{fig/bellhop3d.png}}}
...
The \texttt{aubellhop} repository includes a suite of examples written in a tutorial-style form to illustrate its capabilities and provide starting points for users to perform their own analyses.
...
In this example, we use sound speed data derived from the NOAA World Ocean Atlas by Brian Dushaw\footnote{\url{https://staff.washington.edu/dushaw/WOA/}}. A horizontal slice of this data at constant depth is shown in \Figref{lev_ssp}.

\begin{figure}
\includegraphics{fig/lev_ssp.pdf}
\caption{Sound speed profile (constant depth) derived from NOAA World Ocean Atlas data.}
\label{fig:lev_ssp}
\end{figure}
...
However, all code has been reviewed and checked manually, and for all non-trivial blocks of code has been substantially rewritten and refined.
