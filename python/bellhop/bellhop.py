...
import bellhop.readers as _readers
...
# Populate mappings in sub-modules - this will be called after functions are defined
def _populate_submodules():
    """Populate dependencies in sub-modules after all functions are defined."""
    _readers._populate_mappings(globals())
    _plt._populate_bellhop_deps(check_env2d=check_env2d, spline=spline, interp=_interp, pd=_pd, fi=_fi)
    _plt._populate_pyplot_deps(pyplt=_pyplt, cm=_cm)

# Import reader functions from the readers module
from .readers import read_env2d, read_ssp, read_bty, read_refl_coeff

# Import plot functions from the plot module - these will be available after _populate_submodules is called
def plot_env(*args, **kwargs):
    return _plt.plot_env(*args, **kwargs)

def plot_ssp(*args, **kwargs):
    return _plt.plot_ssp(*args, **kwargs)

def plot_arrivals(*args, **kwargs):
    return _plt.plot_arrivals(*args, **kwargs)

def plot_rays(*args, **kwargs):
    return _plt.plot_rays(*args, **kwargs)

def plot_transmission_loss(*args, **kwargs):
    return _plt.plot_transmission_loss(*args, **kwargs)

def pyplot_env(*args, **kwargs):
    return _plt.pyplot_env(*args, **kwargs)

def pyplot_ssp(*args, **kwargs):
    return _plt.pyplot_ssp(*args, **kwargs)

def pyplot_arrivals(*args, **kwargs):
    return _plt.pyplot_arrivals(*args, **kwargs)

def pyplot_rays(*args, **kwargs):
    return _plt.pyplot_rays(*args, **kwargs)

def pyplot_transmission_loss(*args, **kwargs):
    return _plt.pyplot_transmission_loss(*args, **kwargs)
...
# Now that check_env2d is defined, populate the sub-modules
_populate_submodules()
