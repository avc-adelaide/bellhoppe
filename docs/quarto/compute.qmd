...
By default, `compute()` uses a temporary working directory and deletes all intermediate files after the run.
If you want to keep the generated `.env` and output files, pass `debug=True`.
