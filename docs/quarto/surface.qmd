---
title: "AUBELLHOP: surface properties"
format:
  html:
    code-fold: true
jupyter: python3
...
Option                            Description
------                            -----------
`surface_depth`                   2xN array of (range,topdepth) pairs
...
surf = surf - np.min(surf) # ensure minimum bottom_depth = 0m
env["surface_depth"] = np.column_stack((dist_m, surf))
...
env["surface_depth"] = np.column_stack((x, eta))
...
This profile can be used to set the altimetry data in Bellhop using the `surface_depth` option. For efficiency only 50 rays are shown (@fig-ice-rays). The uneven surface profile creates a complex series of reflections. A simple acousto-elastic surface boundary condition can be set up using parameters from:
