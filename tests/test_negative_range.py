...
def test_positive_receiver_ranges_unchanged():
    ...
    # Store original angle values
    original_min_angle = env["min_angle"]
    original_max_angle = env["max_angle"]
    
    # Verify environment is valid
    bh.check_env2d(env)
    
    # Verify that angle range was NOT modified for positive-only ranges
    assert env['min_angle'] == original_min_angle, "min_angle should not be modified for positive-only ranges"
    assert env['max_angle'] == original_max_angle, "max_angle should not be modified for positive-only ranges"
